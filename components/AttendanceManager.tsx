              ...
              {pendingList.map((item) => {
                ...
                return (
                  <div key={item.idbgd} className="p-5 rounded-[30px] border border-slate-100 bg-slate-50/50 space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      <div className={`grid ${isRange ? 'grid-cols-2' : 'grid-cols-1'} gap-3`}>
                        <div className="space-y-1">
                          <label className="text-[9px] font-black uppercase text-slate-400 ml-2">{isRange ? "Từ ngày" : "Ngày vắng"}</label>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <select value={item.break} onChange={(e) => updatePendingItem(item.idbgd, 'break', e.target.value)} className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold shadow-sm">
                          <option value="N">Cả ngày (N)</option>
                          <option value="S">Buổi Sáng (S)</option>
                          <option value="C">Buổi Chiều (C)</option>
                        </select>
                        <select value={item.lido} onChange={(e) => updatePendingItem(item.idbgd, 'lido', e.target.value)} className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold shadow-sm">
